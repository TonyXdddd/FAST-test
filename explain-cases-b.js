...
export default [
  ...
  },
  {
    ...
    expect: 'submit',
    code: 'btn-contract fu-mimic-count',
    expectText: '<code>form.requestSubmit()</code>, <code>submitter = null</code>',
    why: 'У таких кнопок <code>isSubmitButton = false</code>, то есть submit-кнопок в форме нет. Поле одно — форма отправляется без клика.',
  ...
  },
  {
    ...
    why: '<code>mimicFormEnterBehavior</code> вернул <code>false</code>, значит <code>return !false</code> = <code>true</code>, и FAST не отменяет keydown. Браузер присылает keypress и, если текст менялся, нативный <code>change</code>. Впишите текст и нажмите Enter, затем сравните с кейсом 1.',
  ...
  },
  {
    ...
    why: 'Пока попап открыт, <code>YearPicker.handleKeydownEvent</code> выбирает значение, закрывает попап и возвращает <code>false</code> — до <code>mimicFormEnterBehavior</code> дело не доходит. Когда попап закрыт, вызывается <code>super.handleKeydownEvent</code>. Открыть попап снова — кнопкой «Сбросить».',
